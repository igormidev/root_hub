/* AUTOMATICALLY GENERATED CODE DO NOT MODIFY */
/*   To generate run: "serverpod generate"    */

// ignore_for_file: implementation_imports
// ignore_for_file: library_private_types_in_public_api
// ignore_for_file: non_constant_identifier_names
// ignore_for_file: public_member_api_docs
// ignore_for_file: type_literal_in_constant_pattern
// ignore_for_file: use_super_parameters
// ignore_for_file: invalid_use_of_internal_member

// ignore_for_file: no_leading_underscores_for_library_prefixes
import 'package:serverpod_client/serverpod_client.dart' as _i1;

abstract class ReverseGeocodeCityResult implements _i1.SerializableModel {
  ReverseGeocodeCityResult._({
    this.cityName,
    this.regionName,
    this.countryName,
    this.shortAddress,
    this.formattedAddress,
  });

  factory ReverseGeocodeCityResult({
    String? cityName,
    String? regionName,
    String? countryName,
    String? shortAddress,
    String? formattedAddress,
  }) = _ReverseGeocodeCityResultImpl;

  factory ReverseGeocodeCityResult.fromJson(
    Map<String, dynamic> jsonSerialization,
  ) {
    return ReverseGeocodeCityResult(
      cityName: jsonSerialization['cityName'] as String?,
      regionName: jsonSerialization['regionName'] as String?,
      countryName: jsonSerialization['countryName'] as String?,
      shortAddress: jsonSerialization['shortAddress'] as String?,
      formattedAddress: jsonSerialization['formattedAddress'] as String?,
    );
  }

  String? cityName;

  String? regionName;

  String? countryName;

  String? shortAddress;

  String? formattedAddress;

  /// Returns a shallow copy of this [ReverseGeocodeCityResult]
  /// with some or all fields replaced by the given arguments.
  @_i1.useResult
  ReverseGeocodeCityResult copyWith({
    String? cityName,
    String? regionName,
    String? countryName,
    String? shortAddress,
    String? formattedAddress,
  });
  @override
  Map<String, dynamic> toJson() {
    return {
      '__className__': 'ReverseGeocodeCityResult',
      if (cityName != null) 'cityName': cityName,
      if (regionName != null) 'regionName': regionName,
      if (countryName != null) 'countryName': countryName,
      if (shortAddress != null) 'shortAddress': shortAddress,
      if (formattedAddress != null) 'formattedAddress': formattedAddress,
    };
  }

  @override
  String toString() {
    return _i1.SerializationManager.encode(this);
  }
}

class _Undefined {}

class _ReverseGeocodeCityResultImpl extends ReverseGeocodeCityResult {
  _ReverseGeocodeCityResultImpl({
    String? cityName,
    String? regionName,
    String? countryName,
    String? shortAddress,
    String? formattedAddress,
  }) : super._(
         cityName: cityName,
         regionName: regionName,
         countryName: countryName,
         shortAddress: shortAddress,
         formattedAddress: formattedAddress,
       );

  /// Returns a shallow copy of this [ReverseGeocodeCityResult]
  /// with some or all fields replaced by the given arguments.
  @_i1.useResult
  @override
  ReverseGeocodeCityResult copyWith({
    Object? cityName = _Undefined,
    Object? regionName = _Undefined,
    Object? countryName = _Undefined,
    Object? shortAddress = _Undefined,
    Object? formattedAddress = _Undefined,
  }) {
    return ReverseGeocodeCityResult(
      cityName: cityName is String? ? cityName : this.cityName,
      regionName: regionName is String? ? regionName : this.regionName,
      countryName: countryName is String? ? countryName : this.countryName,
      shortAddress: shortAddress is String? ? shortAddress : this.shortAddress,
      formattedAddress: formattedAddress is String?
          ? formattedAddress
          : this.formattedAddress,
    );
  }
}
